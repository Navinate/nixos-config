...
```bash
just rebuild      # sudo nixos-rebuild switch --flake .#nixtest  (build + activate)
...
```bash
sudo nixos-rebuild switch --flake .#nixtest --rollback
```

## Architecture

This is a flake-based NixOS + Hyprland config targeting a single host (`nixtest`), currently running inside VirtualBox for testing before bare-metal deployment. Inputs are pinned to `nixpkgs 25.11` and `home-manager release-25.11`.

**Two configuration layers:**

- `modules/nixos/` — system-level (runs as root). Imported by `hosts/nixtest/configuration.nix`. Contains: `hyprland.nix` (greetd/tuigreet, PAM, portals, fonts), `audio.nix` (pipewire), `vm.nix` (VirtualBox guest additions — delete this on bare metal).
...
- `hardware-configuration.nix` is machine-generated and gitignored-in-spirit — replace it after a fresh install with `sudo cp /etc/nixos/hardware-configuration.nix hosts/nixtest/`.
